...
**A. 打字练习模块**
- 20 篇中文日常文章（每篇 150–300 字，少标点），随机加载。
...
| 级别 | 功能项 | V1 | 说明 |
| --- | --- | :--: | --- |
| **Must** | 打字练习 - 20 篇中文文章随机加载 | ✅ | 每篇 150–300 字，少标点 |
...
window.TP_ARTICLES = [
  { id: 1,  text: "……" },   // text 为纯中文字符串，150–300 字，少标点
  { id: 2,  text: "……" },
  // … 共 20 篇，id 从 1 到 20 唯一
];
```
- **V1 字段**：`id`（Number，唯一）、`text`（String，150–300 字，少标点，纯中文日常内容）。
...
| B4 | 特殊字符 / 标点 | 不破坏 DOM、正常显示与对比 | Vue 文本插值 `{{ }}` 默认转义；文章少标点纯中文 |
